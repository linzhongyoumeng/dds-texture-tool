...
contextBridge.exposeInMainWorld('ddsTool', {
  ...
  onProcessProgress: (callback) => { const listener = (event, data) => callback(data); ipcRenderer.on('process-progress', listener); return () => ipcRenderer.removeListener('process-progress', listener); },
  onProcessLog: (callback) => { const listener = (event, message, type) => callback(message, type || 'info'); ipcRenderer.on('process-log', listener); return () => ipcRenderer.removeListener('process-log', listener); },
  onScanProgress: (callback) => { const listener = (event, data) => callback(data); ipcRenderer.on('scan-progress', listener); return () => ipcRenderer.removeListener('scan-progress', listener); },
