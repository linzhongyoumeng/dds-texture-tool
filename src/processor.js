...
class DDSProcessor {
  ...
  calculateNewSize(info) {
    ...
    if (scaled) {
      if (this.config.min_width > 0) newW = Math.max(newW, this.config.min_width);
      if (this.config.min_height > 0) newH = Math.max(newH, this.config.min_height);
    }

    if (scaled && align > 1) {
      newW = Math.max(Math.floor(newW / align) * align, align);
      newH = Math.max(Math.floor(newH / align) * align, align);
    }
  ...
  shouldProcess(info) {
    ...
    if (this.config.skip_unchanged === false) {
      const { width: newW, height: newH } = this.calculateNewSize(info);
      return { need: true, newSize: { width: newW, height: newH } };
    }
    ...
    if (this.config.target_format && this.config.force_format) {
      formatChanged = (this.config.target_format.toUpperCase() !== info.format.toUpperCase());
    }

    let mipmapChanged = false;
    if (this.config.mipmaps !== null && this.config.mipmaps !== undefined) {
      mipmapChanged = (this.config.mipmaps !== info.mipmaps);
    }
  ...
  buildCommand(info, outputPath) {
    ...
    if (this.config.mipmaps !== null && this.config.mipmaps !== undefined) {
      cmd.push('-m', String(this.config.mipmaps));
    } else if (this.config.generate_mipmaps) {
      cmd.push('-m');
    }

    const targetFmt = this.config.target_format;
    if (targetFmt && this.config.force_format) {
      cmd.push('-f', targetFmt);
      if (targetFmt.toUpperCase().includes('SRGB') || this.config.srgb === true) {
        cmd.push('-srgb');
      }
    } else if (info.format !== 'UNKNOWN' && SUPPORTED_OUTPUT_FORMATS.includes(info.format)) {
      cmd.push('-f', info.format);
      if (info.format.toUpperCase().includes('SRGB')) cmd.push('-srgb');
    } else if (this.config.srgb === true) {
      cmd.push('-srgb');
    }
  ...
  async processFile(filepath, backupId = null) {
    ...
    try {
      ...
      if (!info) {
        result.skipped = true;
        result.skip_reason = '无法解析 DDS 文件';
        result.success = true;
      ...
      if (!need) {
        result.skipped = true;
        result.skip_reason = '无需处理（尺寸/格式均符合要求）';
        result.success = true;
        result.duration = (Date.now() - startTime) / 1000;
        return result;
      }
      ...
      if (this.config.dry_run) {
        if (this.onLog) this.onLog(`[预览] ${path.basename(filepath)}: ${info.width}x${info.height} -> ${newSize.width}x${newSize.height} (${info.format} -> ${result.new_format})`);
        result.success = true;
        result.duration = (Date.now() - startTime) / 1000;
        return result;
      }
        ...
        const child = execFile(exe, args, { timeout: 300000, maxBuffer: 50 * 1024 * 1024 }, (error, stdout, stderr) => {
          if (error) {
            const errMsg = [
              ...
              error.message,
            ].filter(Boolean).join(' | ') || `texconv 返回码 ${error.code}`;
      ...
      if (!fs.existsSync(outputPath)) {
        ...
        const possibleOutput = path.join(outputDir, baseName + '.dds');
      ...
      if (this.onLog) this.onLog(`[失败] ${path.basename(filepath)}: ${e.message}`);
    }
