...
const SUPPORTED_OUTPUT_FORMATS = [
  ...
  'B8G8R8A8_UNORM', 'B8G8R8A8_UNORM_SRGB', 'R16G16B16A16_FLOAT', 'R32G32B32A32_FLOAT',
  'R8_UNORM', 'A8_UNORM', 'R16_UNORM', 'R16_FLOAT',
];

class DDSParser {
  constructor() { this.DDS_MAGIC = Buffer.from('DDS '); }
  ...
  parse(filepath) {
    try {
      ...
      if (isDx10) {
        ...
        if (rgbBitCount === 32 && rBitmask === 0x00FF0000) fmtName = 'B8G8R8A8_UNORM';
        else if (rgbBitCount === 32) fmtName = 'R8G8B8A8_UNORM';
        else if (rgbBitCount === 24) fmtName = 'R8G8B8_UNORM';
        else if (rgbBitCount === 16) fmtName = 'R5G6B5_UNORM';
      }
      const isCubemap = !!(caps2 & 0x200);
      const isArray = arraySize > 1;
      const actualMipmaps = (flags & 0x20000) ? mipmaps : 0;
      return {
        filepath, width, height, depth: (flags & 0x800000) ? depth : 1,
        mipmaps: actualMipmaps, format: fmtName, dxgi_format: dxgiFormat,
        fourcc: fourcc || null, is_dx10: isDx10, is_cubemap: isCubemap,
        is_array: isArray, array_size: arraySize, file_size: fileSize,
        header_size: 128 + (isDx10 ? 20 : 0), pixel_count: width * height,
        aspect_ratio: height > 0 ? width / height : 0,
      };
    } catch (e) { console.error('解析 DDS 失败:', filepath, e.message); return null; }
  }
